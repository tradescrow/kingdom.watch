...
function currentEpoch(currentBlock: number, expansion: number) {
    ...
    if(diff < 0)
        throw new Error(`Invalid current block ${currentBlock} is less than first block ${expansionEpochData.firstBlock[expansion]}`);
...
function epochStartBlock(epoch: number, expansion: number) {
    return expansionEpochData.firstBlock[expansion] + (expansionEpochData.blocksPerEpoch[expansion] * (epoch - 1))
}

function secondsLeftUntilEpoch(currentBlock: number, epoch: number, expansion: number) {
    return (epochStartBlock(epoch, expansion) - currentBlock) * expansionEpochData.blockSpeedSeconds[expansion]
}

function timeLeftUntilEpochString(currentBlock: number, epoch: number, expansion: number) {
...
function getEpochData(epoch: number, currentBlock: number, expansion: number) {
    return {
        ...
        unlock: Math.min(
            100,
            (epoch * expansionEpochData.unlockWeeklyIncrease[expansion]) + expansionEpochData.unlockStart[expansion] - 2
        ),
...
function calculateEpochData(epochStartBlocks: Array<number>, maxEpochs: number) {
