function timeSpanStringSeconds(seconds, stringIfZero) {
  if (seconds === 0 && stringIfZero.length > 0) return stringIfZero

  if (seconds < 0) return 'past time'
  else if (seconds < 60) return 'imminent'
  else if (seconds < 600) return 'less than 10 minutes'
  ...
  else return 'about ' + (seconds / (30 * 86400)).toFixed(1) + ' months'
}
