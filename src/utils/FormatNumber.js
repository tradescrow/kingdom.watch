function formatNumber(num, prefix, suffix, decimals) {
  ...
  if (!isNaN(decimals)) defaultDecimals = decimals

  let numString

  const absNum = Math.abs(num)
  if (absNum === 0) numString = '0.00'
  else if (absNum < 0.001) numString = absNum.toFixed(6)
  else if (absNum < 0.01) numString = absNum.toFixed(4)
  else if (absNum < 0) numString = absNum.toFixed(3)
  else if (absNum > 1000)
    numString = absNum
      .toFixed(0)
      .toString()
