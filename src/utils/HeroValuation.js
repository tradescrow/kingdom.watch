...
export function valuateAllProfessions(heroData) {
  ...
  for (let prof of professions) scores[prof] = valuateProfession(heroData, prof)
...
export function valuateProfession(heroData, profession) {
  ...
  const stat1val =
    heroData.stats[stat1name] * 1 +
    calcStatGrowth(heroData, stat1name, levelsLeft)
  const stat2val =
    heroData.stats[stat2name] * 1 +
    calcStatGrowth(heroData, stat2name, levelsLeft)

  // Combine them
  let statSum = stat1val + stat2val

  // If this is the main profession of the hero, add 10%
  if (heroData.profession === profession) statSum *= 1.1
